  ...
  return (
    <main className="flex flex-col items-center justify-between w-full min-h-screen space-y-8 ">
      ...
      <HeroComponent
        ...
        buttons={
          homeData?.heroSection?.buttons?.map((button) => ({
            text: button.text ?? "",
            href: button.href ?? "",
          })) || []
        }
      ...
      <FeaturesAndServicesSection
        ...
        facilities={
          homeData?.servicesAndFacilitiesSection?.facilities?.map((f) => ({
            ...f,
      ...
      <TeamBehindSection
        team={
          homeData?.teamBehined?.slice(0, 4).map((member) => ({
            name: member.name ?? "",
